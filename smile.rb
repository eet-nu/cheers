...
class Smile
  ...
  def compose_avatar
    ...
    avatar.add_compose_mask(Magick::Image.read(smile_components[:bg_mask])[0])
    ...
    avatar.add_compose_mask(Magick::Image.read(smile_components[:smile])[0])
    avatar.composite!(smile, 0, 0, Magick::OverCompositeOp)
    avatar.delete_compose_mask
    
    eyes = Magick::Image.new(512, 512) { self.background_color = eyes_color }
    avatar.add_compose_mask(Magick::Image.read(eyes_components[:eyes])[0])
